...
if (isset($_POST['Añadir_alumno'])){
	...
	$insert_query = mysqli_query($conn,"insert into `alumno`(PK_Alumno, nombre, AP_Paterno, AP_Materno, Curp, Fecha_Nac, FK_Direccion, Genero, Celular, Email) 

		VALUES('$matricula','$nombre', '$AP_Paterno', '$AP_Materno', '$Curp', '$Fecha_Nac', '$Direccion','$Genero',' $Celular', '$Email')") or die('query failed');
...
if (isset($_GET['delete'])) {
	...
	$delete_query = mysqli_query($conn,"delete from `alumno` where PK_Alumno = $delete_id");
	if ($delete_query) {	
...
if (isset($_POST['update_alumno'])) {
	...
	$update_celular = $_POST['update_celular'];
	$update_email = $_POST['update_email'];

	$update_query = mysqli_query($conn, "UPDATE `alumno` SET  nombre = '$update_nombre', AP_Paterno = '$update_ap_p', AP_Materno = '$update_ap_m', Curp = '$update_Curp',Fecha_Nac = '$update_fecha',FK_Direccion = '$update_direccion',Genero = '$update_genero', Celular = '$update_celular', Email = '$update_email'  WHERE PK_Alumno = '$update_matricula'");
...
<form action="" method="post" class="add-product-form" enctype="multipart/form-data">
	...
	<input oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); type="number" name="Direccion" maxlength="2"  placeholder="Direccion" class="box" required>
	<input type="text" multiple list="sexo" name="Genero" placeholder="Genero" class="box" required>
	<input oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); type="number" name="Celular" maxlength="10" placeholder="Celular" class="box" required>
	<input type="text" name="Email" placeholder="Correo" class="box" required>
	<input type="submit" value="Añadir alumno" name="Añadir_alumno" class="btn">

	<datalist class="list" id="sexo">
		...
		<tbody>
			<?php 
			 ...
			 <tr>
				...
				<td>
					<a href="menu.php?delete=<?php echo $row['PK_Alumno']; ?>" class="delete-btn" onclick="return confirm('estas seguro?');"> <i class="fas fa-trash"></i>Eliminar</a>
					<a href="menu.php?edit=<?php echo $row['PK_Alumno']; ?>" class="option-btn"> <i class="fas fa-edit"></i>Editar</a>
				</td>
			 </tr>
...
<section class="edit-form-container">
	...
	if(isset($_GET['edit'])){
		$edit_mat = $_GET['edit'];
		$edit_query = mysqli_query($conn,"SELECT * fROM `alumno` WHERE PK_Alumno = $edit_mat ");
	...
	<form action="" method="post" enctype="multipart/form-data">
		<input oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); type="number" maxlength="6"  class="box" required name="update_matricula" value="<?php echo $fetch_edit ['PK_Alumno']; ?>" readonly>
		...
		<input type="text" class="box" required name="update_genero" value="<?php echo $fetch_edit ['Genero']; ?>">
		<input oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength); type="number" class="box" maxlength="10" required name="update_celular" value="<?php echo $fetch_edit ['Celular']; ?>">
